...
-- prepare and qualify candidates, but they cannot create, broaden, revoke, or
-- otherwise mutate published promotion/binding authority.
...
-- Existing promotions predate the distinct authorizer boundary. Their
-- provenance cannot be reconstructed truthfully from database state alone, so
-- refuse the upgrade and require an explicit reconciliation migration.
DO $preexisting_promotions$
BEGIN
    IF EXISTS (SELECT 1 FROM promotions) THEN
        RAISE EXCEPTION
            'preexisting promotions require explicit authorization-continuity reconciliation';
    END IF;
END;
$preexisting_promotions$ LANGUAGE plpgsql;

ALTER TABLE promotions
    ALTER COLUMN authorization_ref SET NOT NULL,
    ADD CONSTRAINT promotions_authorization_ref_format_ck
        CHECK (
            length(authorization_ref) = 71
            AND authorization_ref ~ '^sha256:[0-9a-f]{64}$'
        );

CREATE UNIQUE INDEX promotions_authorization_ref_uq
    ON promotions(authorization_ref);
...
RETURNS trigger AS $$
BEGIN
    IF length(NEW.authorization_ref) <> 71
       OR NEW.authorization_ref !~ '^sha256:[0-9a-f]{64}$' THEN
        RAISE EXCEPTION
            'promotion authorization_ref must be a sha256 content digest';
    END IF;
...
BEGIN
    ...
    -- For the deliberately flat scalar selector contract, JSONB containment
    -- means the binding preserves every approved scope constraint. Additional
    -- key/value constraints make the binding narrower.
...
BEGIN
    ...
    IF p_authorization_ref IS NULL
       OR length(p_authorization_ref) <> 71
       OR p_authorization_ref !~ '^sha256:[0-9a-f]{64}$' THEN
        RAISE EXCEPTION
            'authorization_ref must be a sha256 content digest at the authorizer boundary';
    END IF;
...
-- Keep the already-hardened v2 role recipe intact as an internal base. The
-- current v2 source grants no promotion/binding DML, but the wrapper also
-- verifies that the effective privilege graph did not regain any.
...
BEGIN
    ...
    IF pg_catalog.has_any_column_privilege(
       ...
       OR pg_catalog.has_table_privilege(
        ...
        RAISE EXCEPTION
            'runtime role % retains protected promotion/binding DML authority',
            p_role;
    END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FOREACH t IN ARRAY ARRAY[
        ...
        IF pg_catalog.has_table_privilege(
                p_role, pg_catalog.format('%I.%I', s, t),
...
DECLARE
    ...
    signatures text[] := ARRAY[
        'selector_scope_is_valid(jsonb)',
        'validate_promotion_authority_shape()',
        'validate_injection_binding_authority()',
        'protect_injection_binding_restriction()',
