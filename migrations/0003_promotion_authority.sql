...
-- Existing promotions predate the distinct authorizer boundary, so this
-- migration cannot truthfully manufacture authorization provenance for them.
-- Refuse the upgrade and require an explicit reconciliation migration instead
-- of silently treating legacy promotions as authorized.
...
ALTER TABLE promotions
    ...
    ADD CONSTRAINT promotions_authorization_ref_format_ck
        CHECK (authorization_ref ~ '^sha256:[0-9a-f]{64}RETURNS boolean AS $$
BEGIN
    IF p_scope IS NULL
...
RETURNS trigger AS $$
BEGIN
    IF NEW.authorization_ref !~ '^sha256:[0-9a-f]{64}    IF NOT selector_scope_is_valid(NEW.activation_scope) THEN
        RAISE EXCEPTION
...
BEGIN
    ...
    IF p_authorization_ref IS NULL
       OR p_authorization_ref !~ '^sha256:[0-9a-f]{64}
    RETURN QUERY
...
COMMIT;
 THEN
        RAISE EXCEPTION 'promotion authorization_ref must be a sha256 content digest';
    END IF;
